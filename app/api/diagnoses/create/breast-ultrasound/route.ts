...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Processing breast ultrasound image...")
    const aiResult: {
      success: boolean
      data?: any
      modelType: string
...
if (Array.isArray(aiResult.data)) {
  ...
  for (let i = 0; i < aiResult.data.length; i++) {
    ...
    console.log("Per image analysis:", imageAnalysis)
  }
}

const end = performance.now()
const durationMs = end - start
console.log("Image processing duration:", durationMs, "ms")


    if (!aiResult.success) {
      return NextResponse.json({ error: aiResult.error || "Failed to process breast ultrasound" }, { status: 500 })
    }
