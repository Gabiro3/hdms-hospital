...
import {
  getPendingRecordRequests,
  getSharedRecords,
  updateRecordRequestStatus,
} from "@/services/patient-records-service"

interface PatientRecordsManagementProps {
  patientId: string
  hospitalId: string
}

export default function PatientRecordsManagement({ patientId, hospitalId }: PatientRecordsManagementProps) {
    ...
    const fetchData = async () => {
      ...
      try {
        if (activeTab === "pending") {
          const { requests, error } = await getPendingRecordRequests(patientId, hospitalId)
                ...
                <TableBody>
                  {pendingRequests.map((request) => (
                    <TableRow key={request.id}>
                      <TableCell className="font-medium">{request.requesting_hospital_name}</TableCell>
